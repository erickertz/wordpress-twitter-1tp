...
class Wordpress_Twitter_1TP {
	...
	private function save_tweet_acf($pageId,$status){
		...
		$row_limit = $acfRepeaterStructure['fields'][0]['row_limit'];
		$value = get_field($field_key, $pageId, true);
		if(is_numeric($row_limit)){
			$value = array_slice($value,0,$row_limit-1,true);
		}
		$valueNew = array();
		$valueNew[] = $newTweet;
